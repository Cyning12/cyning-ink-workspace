...
| notes | 对照子仓 commit `47a6f9e`；审查侧重 freeze_id、§3.2 N/A、FP-G/H/I、§6/§8/§10 一致性 |
...
执行帽已合入子仓提交（供对照 diff，非 task freeze_id）：
- 仓库：ai-ink-brain-api-python，commit：47a6f9e
...
审查侧重（本轮人工追加，与 task 一致即可）：
- freeze_id 行不得出现 Actions run id；§3.2 浏览器默认 N/A 不得入主结论。
- FP-G/FP-H/FP-I：专文 §9 与 §4 锚点、CLI 真值仅为 tech_graph_graph_export.py / tech_graph_token_estimate.py、计时 A/B 与 §2 代号 A/B 不得混排。
- 父文档 §6 勾选与「总对比表主真值在专文」PR 一句是否与 task §8「已选」及 §10 自检结论一致；§4.2 中 PR/CI 与归档仍为 [ ] 是否属预期阻塞。
```
