...
| 字段 | 值 |
|------|-----|
| hat_id | 10（执行帽 / 文档收敛；与专题 draft invoke 同帽族） |
| template | 用户结构化指令（Harness 执行帽 / 文档收敛）；非 `TEMPLATE-*` 全量粘贴 |
...
| created_utc_or_local | 2026-05-14 |
| notes | 消除 R1 / 前端 graph CI 必绿 矛盾；两仓 task 写入同一 `freeze_id`；failure_paths 与 contract_check 并行不合并 |
...
【硬约束】
- 已存在帽 20 短评结论：须先消除 docs/tech_graph/SPEC/json_graph/scheme_1_graph_json.md「仍待执行方补全」与 docs/tech_graph/改进方向.md v1.1.1（R1）及 ai-ink-brain/content/tasks/active/task_engineering_tech_graph_graph_json_export_v1.md 之间关于「前端 graph.json CI 是否必绿」的矛盾；并在两子仓 task 写入单行 freeze_id。
