...
**Verify（验证）**：**P0（2026-05-13）** 已落盘 **前端** `quality.yml`（`pnpm` install → lint → build）与 **后端** `pytest.yml`（`pytest -m "not intent_eval and not intent_benchmark"` + CI 环境 dummy Key / 澄清闸关闭）。**P1（2026-05-13）** 起前端同 workflow 增补 **`pnpm test`**（Vitest）。后续仍按 **§4** 维护 checklist 与测评对照。
...
- **CI 背压（P0 已落地）**：Ink 前后端默认分支 PR 上具备 **lint+build** 与 **pytest** 门禁；**P1（2026-05-13）** 起前端 **`quality`** 增补 **`pnpm test`**（Vitest）。
...
| 2026-05-13 | P1：`quality` 增 **`pnpm test`**（Vitest）；§4 P0/P1 与 §8 Verify 摘要同步；根 `AGENTS.md` §8 链 **`quality`** / **`lint-and-build`** / **Test** step |
