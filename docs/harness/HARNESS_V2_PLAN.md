...
**Verify（验证）**：**P0（2026-05-13）** 已落盘 **前端** `quality.yml`（`pnpm` install → lint → build）与 **后端** `pytest.yml`（`pytest -m "not intent_eval and not intent_benchmark"` + CI 环境 dummy Key / 澄清闸关闭）。后续仍按 **§4 P1** 扩前端单测、统一「合并前必绿」清单等。

### 0.2 V2 目标（可验收）

- **分层真源**：工作区级 Harness 文档与入口在 `Projects/docs/harness/` 与根 `AGENTS.md`；各栈 **命令与 CI** 仍在子仓落盘，避免双份漂移。
- **SDD 不变**：SPEC → task → 审查 → 执行 → 自检 → 复检 → 人工 checklist 的流程继续由各仓任务目录承载。
- **显式 TDD 档位**：非「默认全盘 TDD」，而在 task 中 **声明** `test_strategy`，执行与复检据此门闸。
- **CI 背压（P0 已落地）**：Ink 前后端默认分支 PR 上具备 **lint+build** 与 **pytest** 门禁；P1 仍含前端单测与 checklist 扩面。
...
| **Verify** | 合并前机器可重复验证 | **前端：PR 上 lint + build**；**后端：PR 上 pytest（排除 slow/真 LLM marker）**；文档化「合并前必绿」清单 |

**帽子（身份 Prompt）**：属于 **Guides** 层，与三支柱 **叠加**——短 system 前缀定义角色、输入裁剪、拒开工条件；**不能替代** CI。各帽详细句柄可后续单文件维护（如 `docs/harness/prompts/*.md`），V2 初版仅在 §3 列目录与硬规则摘要。
...
| 帽子 | 硬规则摘要（执行时须满足） |
|------|---------------------------|
| **需求 / 任务分析** | 任务单须含 **验收可观测**、**失败路径**、**非范围**、依赖为链接；发现文档矛盾列矛盾点，不调和叙事 |
| **规格 / 任务审查** | 只输出缺口与可测性意见；契约变更标 **冻结点升级** |
| **执行编码** | 遵守 task **必读列表**与 **test_strategy**；缺失败路径或验收不可执行 → **仅输出阻塞清单**，不写业务代码；SPEC 错误走变更请求，不静默扩 scope |
| **自检（执行者）** | 必须运行 task 所列 **命令** 并保留 **原始输出要点** |
| **独立复检** | 输入以 **diff + 日志 + 验收表** 为主；逐项 pass/fail，fail 须引用证据 |
...
1. **前端 `ai-ink-brain`**：`.github/workflows/quality.yml` — `pull_request` + `push`（`main`、`production`）：`pnpm install --frozen-lockfile` → `pnpm lint` → `pnpm build`。
...
3. **前端最小单测**：引入 Vitest（或 Playwright smoke 择一），覆盖关键 BFF/工具函数；与 `test_strategy` 对齐。
4. **根 `AGENTS.md` 或本文件**：维护 **「合并前必绿」**  checklist 链接到各仓 workflow 名称与本地等价命令。

### P2

5. **可选独立 Verification job**：仅跑测试 + 静态分析，与「写代码」Agent 权限分离（组织层面落地）。
6. **`docs/harness/prompts/`**：各帽独立 md，版本化；与本规划 §3 同步修订。
...
`Harness`、`Verify`、`test_strategy`、`freeze_id`、`failure_paths`、`gates_before_code`、`CI`、`帽子`、`SDD`、`TDD`
