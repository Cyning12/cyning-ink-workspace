...
| **C**（方案2 深化 · 非重跑 A/B） | **D** `CTX_V2_QUERY` vs **E** `CTX_DUAL_MD` | 结论 [`conclusion_gate_c_v2_dual_track_v1_zh.md`](../../ai-ink-brain-api-python/docs/diary/jsonPKmermaid/reports/conclusion_gate_c_v2_dual_track_v1_zh.md)（`accepted`）；canonical run `gate_ctx_c_v1_batch_20260518_052803`；**freeze_id** `TECH_GRAPH_GATE_C_FREEZE_20260518_V1_0`。**不推翻** B 已采纳的 **CTX_QUERY / `graph_query` machine 默认**；**E** 为人读/按需双轨，**非** Agent machine 默认 |
| **方案3 立项** | 依赖 **B** 通过 + **R2**（**不**以 C 替代 B） | 在子仓 task 中引用 B 的结论与 **freeze_id**；再动 Neo4j 代码与运维 |
...
| 方案2 | 内存图查询工具 | `tech_graph_graph_query.py` + Cursor 工具调用 | 中～大（节点 <2000） | 方案1 + **闸口 A** 对比实验归档；深化实验 **闸口 C**（D vs E）已归档，**不改变** 方案3 对 **闸口 B** 的依赖 |
...
- [x] **闸口 C**（**D** `CTX_V2_QUERY` vs **E** `CTX_DUAL_MD`）**已完成** — 结论：`ai-ink-brain-api-python/docs/diary/jsonPKmermaid/reports/conclusion_gate_c_v2_dual_track_v1_zh.md`（`freeze_id` `TECH_GRAPH_GATE_C_FREEZE_20260518_V1_0` · canonical run `gate_ctx_c_v1_batch_20260518_052803` · gate_c task · **维持** B 的 CTX_QUERY machine 默认；E 为人读/按需双轨 · **禁止**重跑 `gate_ctx_c` batch 主实验）。
...
| v1.1.3+P3 | 2026-05-18 | **闸口 C** 对比实验门闸表 + §2.7 验收勾选；链 `conclusion_gate_c_v2_dual_track_v1_zh.md` / `TECH_GRAPH_GATE_C_FREEZE_20260518_V1_0`；**不**替代 B 默认与 **R2** 方案3 时序 |
