...
| 决议 ID | 内容 |
| --- | --- |
| **R1 图谱目录（Ink）** | 前后端统一采用子仓内 **`docs/_tech_graph/`**。前端 **`ai-ink-brain`** 若仍存在历史 **仓根** `_tech_graph/`，须先通过 **独立子仓 task** 完成目录迁入 `docs/_tech_graph/`（`git mv` 级移动 + 全仓引用与 CI/规则同步），再以前端路径绑定方案1 脚本与门禁。 |
